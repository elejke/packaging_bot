...
def add_new_product_ask_first(bot, update, agent, chat, botuser):
    reply = (["Привет, я Эко-бот, и я помогу тебе зарегистрировать новый продукт"],
             {'reply_markup': ReplyKeyboardMarkup([['Cфотографировать штрих код или ввести номер'],
                                                   ['Ввести вручную название'],
...
def add_new_product_cross(bot, update, agent, chat, botuser):
    if (update.message.text == 'Cфотографировать штрих код или ввести номер'):# or (update.message.text == 'Закончить'):
    ...
    #     reply = "Введите номер штрих-кода"
        ...
        reply = (["Пожалуйста, найдите продукт в каталоге"],
            ...
            if product_id is None:
                ...
                reply = (['%s\nШтрих-код: %d' % (product_name, product_number)],
                         {'reply_markup': ReplyKeyboardMarkup([['Да'],
                                                               ['Нет, ввести штрих-код'],
                                                               ['Нет, ввести название']], one_time_keyboard=True)})
                next_state = Chat.ADD_NEW_PRODUCT_CROSS_PRODUCT_NAME
                upd_dict = {'product_name': product_name,
...
def add_new_product_wait_for_number(bot, update, agent, chat, botuser):
    ...
    reply = (['%s\nШтрих-код: %d' % (product_name, product_number)],
             {'reply_markup': ReplyKeyboardMarkup([['Да'],
                                                   ['Нет, ввести название вручную']], one_time_keyboard=True)})
...
def add_new_product_wait_for_product_name(bot, update, agent, chat, botuser):
    ...
    if update.message.text.startswith('Арт.'):
        ...
        product_number = int(product_number)
        # except:
        #     reply = (["Пожалуйста, выберете товар из каталога"],
        #              {'reply_markup': InlineKeyboardMarkup(
        #                  [
    ...
    if ('Здесь нет моего продукта' in product_name):
        ...
        if 'product_number' in chat_meta.keys():
            ...
            reply = (["Возможно, в нашей базе еще нет этого товара. Вы можете ввести сейчас название нового товара или попробовать поискать в нашем каталоге еще раз."],
                     {'reply_markup': InlineKeyboardMarkup(
                         [
                             # [InlineKeyboardButton(text="Вернуться в главное меню")],
                          [InlineKeyboardButton(text="Войти в каталог",
        ...
        reply = (['Правильно ли мы определили продукт?\n%s\n%s: %s' % (product_name,
                                                                       'Артикул' if art_flag else 'Штрих-код',
                                                                       product_number)],
                 {'reply_markup': ReplyKeyboardMarkup([['Да'],
                                                       ['Нет, ввести штрихкод'],
                                                       ['Нет, ввести название']], one_time_keyboard=True)})
...
def add_new_product_confirm_product_name(bot, update, agent, chat, botuser):
    ...
    reply = (["Продукт определен верно?"],
            # ["Вы подтверждаете, что это %s?"%productname],
             {'reply_markup': ReplyKeyboardMarkup([['Да'],
                                                   ['Нет, ввести название вручную']], one_time_keyboard=True)})
    return reply, Chat.ADD_NEW_PRODUCT_CROSS_PRODUCT_NAME, {"":""}
...
def add_new_product_cross_product_name(bot, update, agent, chat, botuser):
    if (update.message.text == 'Да'):
        ...
        reply = (["Пожалуйста, выберите нужную упаковку в каталоге"],
                 {'reply_markup': InlineKeyboardMarkup(
                                                [[InlineKeyboardButton(text="Войти в каталог",
                                                                       switch_inline_query_current_chat="Упаковки: ")]])})
...
def add_new_product_ask_type_of_wrapper(bot, update, agent, chat, botuser):
    ...
    reply = (['Введите имя производителя'],
...
def add_new_product_wait_for_manufacturer_name(bot, update, agent, chat, botuser):
    if update.message.text != 'Пропустить':
        ...
        manufacturer_name = ''
    reply = (['Введите почту производителя'],
             {'reply_markup': ReplyKeyboardMarkup([['Пропустить']], one_time_keyboard=True)})
...
def add_new_product_wait_for_manufacturer_email(bot, update, agent, chat, botuser):
    if update.message.text != 'Пропустить':
        ...
        manufacturer_email = ''
    reply = (['Введите адрес производителя'],
             {'reply_markup': ReplyKeyboardMarkup([['Пропустить']], one_time_keyboard=True)})
 ...
 #            {'reply_markup': ReplyKeyboardMarkup([['Пропустить']], one_time_keyboard=True)})
    ...
    if 'user_name' not in meta_data.keys():
        ...
        reply = (['Проверьте правильность введенных данных',
                 ...
                 {'reply_markup': ReplyKeyboardMarkup([['Все верно'],
                                                       ['Исправить информацию о прозводителе'],
...
def add_new_product_wait_for_user_email(bot, update, agent, chat, botuser):
    if update.message.text != 'Пропустить':
        ...
        user_email = ''
    reply = (['Проверьте правильность введенных данных',
              add_new_product_string_generation(chat, user_email=user_email)[0]],
             {'reply_markup': ReplyKeyboardMarkup([['Все верно'],
                                                   ['Исправить информацию о прозводителе'],
                                                   ['Исправить информацию о себе'],
                                                   ['Заполнить товар заново']], one_time_keyboard=True)})
...
def add_new_product_wait_for_data_check(bot, update, agent, chat, botuser):
    if update.message.text == 'Все верно':
        ...
        if 'absolutely_new_product' in meta_data.keys():
            ...
            data = {
                'name': meta_data['product_name'],  # обязательный
                bar_code_type: bar_code,
            ...
            reply = (['Мы сохранили добавленный вами продукт! Надеемся, что вы отправите производителю обращение с просьбой, заменить упаковку.'],
                     {'reply_markup': ReplyKeyboardMarkup([['Получить текст петиции'],
                                                           ['Подписать коллективное обращение'],
            ...
            bar_code_type = 'utkonos_id' if art_flag else 'upc' if len(str(art_flag)) == 12 else 'ean'
            data = {
                'product_id':  meta_data['product_name'], # обязательный
        ...
        meta = chat.get_meta()
        for key in ['product_name', 'product_number', 'types_of_wrapper', 'number_of_wrapper',
                    'manufacturer_name', 'manufacturer_email', 'manufacturer_address', 'manufacturer_phone',
                    'user_name', 'user_email']:
            meta.pop(key)
        chat.save_meta(meta)
        reply = (["Выберете поиск товар по штрих коду или по название"],
                 {'reply_markup': ReplyKeyboardMarkup([['Cфотографировать штрих код или ввести номер'],
                                                       ['Ввести вручную название'],
                                                       # ['Ввести вручную номер штрих-кода']
                                                       ], one_time_keyboard=True)})
...
def add_new_product_petition_and_quit(bot, update, agent, chat, botuser):
    if update.message.text == 'Получить текст петиции':
        ...
        reply = (["Чтобы подписать обращение, пожалуйста, введите свой e-mail повторно. Внимание! Этот e-mail должен совпадать с e-mail'ом, введеным ранее, в противном случае ваша заявка в коллективное бращение будет отклонена."],
...
def add_new_product_sign_appeal(bot, update, agent, chat, botuser):
    ...
    if signing_email != user_email:
        reply = (["Простите, но ваши e-mail'ы не свопадают"],
                 {'reply_markup': ReplyKeyboardMarkup([['Получить текст петиции'],
                                                       ['Подписать коллективное обращение'],
                                                       ['Вернуться в главное меню']], one_time_keyboard=True)}
                 )
...
state_map = {
    "__meta__": {
        "caption": 'Зарегистрировать новый продукт',
        "entry_state": Chat.ADD_NEW_PRODUCT_ASK_FIRST,
        "entry_point": add_new_product_ask_first,
    },
    "states": {
        Chat.ADD_NEW_PRODUCT_ASK_FIRST: [MessageHandler(Filters.text, add_new_product_ask_first)],
        ...
        Chat.ADD_NEW_PRODUCT_CONFIRM_PRODUCT_NAME: [
            MessageHandler(Filters.text, add_new_product_confirm_product_name)
        ],
        Chat.ADD_NEW_PRODUCT_CROSS_PRODUCT_NAME: [
            MessageHandler(Filters.text, add_new_product_cross_product_name)
        ],
        ...
        Chat.ADD_NEW_PRODUCT_WAIT_FOR_USER_NAME: [
            MessageHandler(Filters.text, add_new_product_wait_for_user_name)
        ],
        Chat.ADD_NEW_PRODUCT_WAIT_FOR_USER_EMAIL: [
            MessageHandler(Filters.text, add_new_product_wait_for_user_email)
        ],
        Chat.ADD_NEW_PRODUCT_WAIT_FOR_DATA_CHECK: [
            MessageHandler(Filters.text, add_new_product_wait_for_data_check)
        ],
        Chat.ADD_NEW_PRODUCT_PETITION_AND_QUIT: [
            MessageHandler(Filters.text, add_new_product_petition_and_quit)
        ],
        Chat.ADD_NEW_PRODUCT_SIGN_APPEAL: [
            MessageHandler(Filters.text, add_new_product_sign_appeal)
        ],
